...
import radioImg from "../images/radioloLogin.svg";
...
const RadiologistLogin = () => {
  ...
  const postData = async (e) => {
    ...
    const res = await fetch("http://localhost:4000/radiologist/login", {
      method: "POST",
      headers: {
    ...
    if (res.status === 201) {
      dispatch(radiologistId({ currentUser: data._id }));
      navigate("/radiologistRoutes/");
    }
  };
  return (
    <div>
